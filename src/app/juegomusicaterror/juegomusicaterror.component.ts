...
@Component({
  selector: 'app-juegomusicaterror',
  templateUrl: './juegomusicaterror.component.html',
  styleUrls: ['./juegomusicaterror.component.scss'],
  animations: [
    trigger('slideDownUp', [
...
export class JuegomusicaterrorComponent implements OnInit {
  datos!: Juegomusica[];
  respuesta: string = '';
  intentos: number = 0;
  puntos: number = 0;
 ...
 if (nombresPeliculasCookie) {
  this.nombresPeliculas = this.nombresPeliculas.map((pelicula, index) => ({
    ...
    if (!this.nombresPeliculas || this.nombresPeliculas.length === 0) {
      ...
      this.servicioService.postDatoRankingMusicaTerror(nuevo).subscribe((datos) => {
