...
$puntos=$params->puntos;
	...
	//En la siguiente consulta cogemos los puntosImagenFantasia de la tabla ranking del codigoranking cogido en la anterior consulta
	$consultaPuntos = $mbd->prepare("SELECT PuntosImagenFantasia FROM ranking WHERE CodigoRanking = :CodigoRanking");
    $consultaPuntos->bindParam(':CodigoRanking', $id);
	$consultaPuntos->execute();
	$resultadoPuntos = $consultaPuntos->fetch(PDO::FETCH_ASSOC);
	if ($resultadoPuntos) {
        $puntosActuales = $resultadoPuntos['PuntosImagenFantasia'];
        ...
        if ($puntos > $puntosActuales) {
            ...
            $consultaActualizarPuntos = $mbd->prepare("UPDATE ranking SET PuntosImagenFantasia = :PuntosImagenFantasia WHERE CodigoRanking = :CodigoRanking");
            $consultaActualizarPuntos->bindParam(':CodigoRanking', $id);
            $consultaActualizarPuntos->bindParam(':PuntosImagenFantasia', $puntos);
		...
		$sentencia = $mbd->prepare("INSERT INTO ranking (CodigoRanking, PuntosImagenFantasia ) VALUES (:CodigoRanking, :PuntosImagenFantasia)");
		$sentencia->bindParam(':CodigoRanking', $id);
		$sentencia->bindParam(':PuntosImagenFantasia', $puntos);
