...
$puntos=$params->puntos;
	...
	//En la siguiente consulta cogemos los PuntosMusicaFiccion de la tabla ranking del codigoranking cogido en la anterior consulta
	$consultaPuntos = $mbd->prepare("SELECT PuntosMusicaFiccion FROM ranking WHERE CodigoRanking = :CodigoRanking");
    $consultaPuntos->bindParam(':CodigoRanking', $id);
	$consultaPuntos->execute();
	$resultadoPuntos = $consultaPuntos->fetch(PDO::FETCH_ASSOC);
	if ($resultadoPuntos) {
        $puntosActuales = $resultadoPuntos['PuntosMusicaFiccion'];
        ...
        if ($puntos > $puntosActuales) {
            ...
            $consultaActualizarPuntos = $mbd->prepare("UPDATE ranking SET PuntosMusicaFiccion = :PuntosMusicaFiccion WHERE CodigoRanking = :CodigoRanking");
            $consultaActualizarPuntos->bindParam(':CodigoRanking', $id);
            $consultaActualizarPuntos->bindParam(':PuntosMusicaFiccion', $puntos);
		...
		$sentencia = $mbd->prepare("INSERT INTO ranking (CodigoRanking, PuntosMusicaFiccion ) VALUES (:CodigoRanking, :PuntosMusicaFiccion)");
		$sentencia->bindParam(':CodigoRanking', $id);
		$sentencia->bindParam(':PuntosMusicaFiccion', $puntos);
