...
$puntos=$params->puntos;
	...
	//En la siguiente consulta cogemos los PuntosImagenFiccion de la tabla ranking del codigoranking cogido en la anterior consulta
	$consultaPuntos = $mbd->prepare("SELECT PuntosImagenFiccion FROM ranking WHERE CodigoRanking = :CodigoRanking");
    $consultaPuntos->bindParam(':CodigoRanking', $id);
	$consultaPuntos->execute();
	$resultadoPuntos = $consultaPuntos->fetch(PDO::FETCH_ASSOC);
	if ($resultadoPuntos) {
        $puntosActuales = $resultadoPuntos['PuntosImagenFiccion'];
        ...
        if ($puntos > $puntosActuales) {
            ...
            $consultaActualizarPuntos = $mbd->prepare("UPDATE ranking SET PuntosImagenFiccion = :PuntosImagenFiccion WHERE CodigoRanking = :CodigoRanking");
            $consultaActualizarPuntos->bindParam(':CodigoRanking', $id);
            $consultaActualizarPuntos->bindParam(':PuntosImagenFiccion', $puntos);
		...
		$sentencia = $mbd->prepare("INSERT INTO ranking (CodigoRanking, PuntosImagenFiccion ) VALUES (:CodigoRanking, :PuntosImagenFiccion)");
		$sentencia->bindParam(':CodigoRanking', $id);
		$sentencia->bindParam(':PuntosImagenFiccion', $puntos);
