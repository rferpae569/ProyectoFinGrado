...
$puntos=$params->puntos;
	...
	//En la siguiente consulta cogemos los PuntosMusicaTerror de la tabla ranking del codigoranking cogido en la anterior consulta
	$consultaPuntos = $mbd->prepare("SELECT PuntosMusicaTerror FROM ranking WHERE CodigoRanking = :CodigoRanking");
    $consultaPuntos->bindParam(':CodigoRanking', $id);
	$consultaPuntos->execute();
	$resultadoPuntos = $consultaPuntos->fetch(PDO::FETCH_ASSOC);
	if ($resultadoPuntos) {
        $puntosActuales = $resultadoPuntos['PuntosMusicaTerror'];
        ...
        if ($puntos > $puntosActuales) {
            ...
            $consultaActualizarPuntos = $mbd->prepare("UPDATE ranking SET PuntosMusicaTerror = :PuntosMusicaTerror WHERE CodigoRanking = :CodigoRanking");
            $consultaActualizarPuntos->bindParam(':CodigoRanking', $id);
            $consultaActualizarPuntos->bindParam(':PuntosMusicaTerror', $puntos);
		...
		$sentencia = $mbd->prepare("INSERT INTO ranking (CodigoRanking, PuntosMusicaTerror ) VALUES (:CodigoRanking, :PuntosMusicaTerror)");
		$sentencia->bindParam(':CodigoRanking', $id);
		$sentencia->bindParam(':PuntosMusicaTerror', $puntos);
