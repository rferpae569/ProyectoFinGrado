...
$puntos=$params->puntos;
	...
	//En la siguiente consulta cogemos los puntosImagenTerror de la tabla ranking del codigoranking cogido en la anterior consulta
	$consultaPuntos = $mbd->prepare("SELECT PuntosImagenTerror FROM ranking WHERE CodigoRanking = :CodigoRanking");
    $consultaPuntos->bindParam(':CodigoRanking', $id);
	$consultaPuntos->execute();
	$resultadoPuntos = $consultaPuntos->fetch(PDO::FETCH_ASSOC);
	if ($resultadoPuntos) {
        $puntosActuales = $resultadoPuntos['PuntosImagenTerror'];
        ...
        if ($puntos > $puntosActuales) {
            ...
            $consultaActualizarPuntos = $mbd->prepare("UPDATE ranking SET PuntosImagenTerror = :PuntosImagenTerror WHERE CodigoRanking = :CodigoRanking");
            $consultaActualizarPuntos->bindParam(':CodigoRanking', $id);
            $consultaActualizarPuntos->bindParam(':PuntosImagenTerror', $puntos);
		...
		$sentencia = $mbd->prepare("INSERT INTO ranking (CodigoRanking, PuntosImagenTerror ) VALUES (:CodigoRanking, :PuntosImagenTerror)");
		$sentencia->bindParam(':CodigoRanking', $id);
		$sentencia->bindParam(':PuntosImagenTerror', $puntos);
