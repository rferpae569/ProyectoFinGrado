...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasImagenTerror en uno
            $mbd->query("UPDATE numjugadas SET JugadasImagenTerror = JugadasImagenTerror + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasImagenTerror FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
