...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasPreguntasTerror en uno
            $mbd->query("UPDATE numjugadas SET JugadasPreguntasTerror = JugadasPreguntasTerror + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasPreguntasTerror FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
