...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna jugadasspoiler en uno
            $mbd->query("UPDATE numjugadas SET JugadasSpoiler = JugadasSpoiler + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasSpoiler FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
