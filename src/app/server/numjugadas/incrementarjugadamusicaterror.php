...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasMusicaTerror en uno
            $mbd->query("UPDATE numjugadas SET JugadasMusicaTerror = JugadasMusicaTerror + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasMusicaTerror FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
