...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasImagenFantasia en uno
            $mbd->query("UPDATE numjugadas SET JugadasImagenFantasia = JugadasImagenFantasia + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasImagenFantasia FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
