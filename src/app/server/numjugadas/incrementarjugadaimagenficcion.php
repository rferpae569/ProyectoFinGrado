...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasImagenFiccion en uno
            $mbd->query("UPDATE numjugadas SET JugadasImagenFiccion = JugadasImagenFiccion + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasImagenFiccion FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
