...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasMusicaFiccion en uno
            $mbd->query("UPDATE numjugadas SET JugadasMusicaFiccion = JugadasMusicaFiccion + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasMusicaFiccion FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
