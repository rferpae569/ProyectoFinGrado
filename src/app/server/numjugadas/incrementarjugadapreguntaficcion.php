...
try {
    ...
    if ($res->errorCode() == 0) {
        ...
        if (!empty($rows)) {
            ...
            // Consulta para incrementar la columna JugadasPreguntasFiccion en uno
            $mbd->query("UPDATE numjugadas SET JugadasPreguntasFiccion = JugadasPreguntasFiccion + 1 WHERE Codigojugadas = $codigoJugadas");

            // Obtener los datos actualizados después de la actualización
            $resUpdated = $mbd->query("SELECT Codigojugadas, JugadasPreguntasFiccion FROM numjugadas WHERE Codigojugadas = $codigoJugadas");
