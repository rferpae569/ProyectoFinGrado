...
@Component({
  selector: 'app-juegomusicaficcion',
  templateUrl: './juegomusicaficcion.component.html',
  styleUrls: ['./juegomusicaficcion.component.scss'],
  animations: [
    trigger('slideDownUp', [
...
export class JuegomusicaficcionComponent implements OnInit {
  ...
  //Creamos las variables correspondientes
   ...
   ngOnInit() {
    ...
    this.servicioService
      .getDatosPeliculaPistaMusicaFiccion()
      .subscribe((datos) => {
  ...
  seleccionarPalabraSecreta() {
    ...
    if (!this.nombresPeliculas || this.nombresPeliculas.length === 0) {
      ...
      this.servicioService
        .postDatoRankingMusicaFiccion(nuevo)
        .subscribe((datos) => {
  ...
  enviarRespuesta() {
    ...
    // Obtenemos las canciones de las películas desde la cookie 'peliculas'
