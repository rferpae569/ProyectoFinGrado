...
@Component({
  selector: 'app-juegopreguntafantasiadosj',
  templateUrl: './juegopreguntafantasiadosj.component.html',
  styleUrls: ['./juegopreguntafantasiadosj.component.scss'],
  animations: [
    trigger('slideDownUp', [
...
export class JuegopreguntafantasiadosjComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.servicioService.getDatosPeliculaPistaPreguntaFantasia().subscribe((datos) => {
  ...
  constructor(
    ...
    if (!sessionCookieExists) {
      this.servicioService.getDatosPeliculaPreguntaFantasia().subscribe((datos) => {
        this.datos = datos;
  ...
  seleccionarPalabraSecreta() {
    ...
    if (!this.preguntasPeliculas || this.preguntasPeliculas.length === 0) {
      ...
      this.servicioService.postDatoRankingPreguntaFantasia(nuevo).subscribe((datos) => {
        console.log('Datos enviados al servidor:', datos);
      });

      //Enviamos los datos del jugador 2 al servidor
      this.servicioService
        .postDatoRankingPreguntaFantasia(nuevo2)
        .subscribe((datos) => {
          console.log('Datos enviados al servidor:', datos);
        });
  ...
  enviarRespuesta() {
    ...
    if (this.Respuesta.toLowerCase() === juegoActual.toLowerCase()) {
      ...
      if (this.intentos <= -1) {
        ...
        this.servicioService
          .postDatoRankingPreguntaFantasia(nuevo)
          .subscribe((datos) => {
            console.log('Datos enviados al servidor:', datos);
          });
